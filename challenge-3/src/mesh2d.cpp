...
namespace challenge3 {
    ...
    Mesh2D::Mesh2D(const Domain & domain_, size_t nx_, size_t ny_) : domain{domain_},nx{nx_},ny{ny_} { 
        hx = (domain.getMaxX()-domain.getMinX())/nx; ///< setting x spacing according to number of points wanted
        hy = (domain.getMaxY()-domain.getMinY())/ny; ///< setting y spacing according to number of points wanted
    };

    ///constructor given the domain and the spacing along each direction
    Mesh2D::Mesh2D(const Domain & domain_, double hx_, double hy_) : Mesh2D(domain_,static_cast<size_t>(std::ceil((domain_.getMaxX() - domain_.getMinX()) / hx_)),
             static_cast<size_t>(std::ceil((domain_.getMaxY() - domain_.getMinY()) / hy_))) {
